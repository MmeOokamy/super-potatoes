...
def oa_index():
    ...
    for theme in themes:
        ...
        arts = Articles.query.filter(
            Articles.article_theme_id == theme.id).all()
        t['articles'] = arts
        menu.append(t)
...
def oa_theme():
    ...
    if form.validate_on_submit():
        existing_theme = Themes.query.filter_by(
            theme_title=form.theme_title.data).first()
...
def oa_article():
    ...
    form.article_theme_id.choices = [
        (t.id, t.theme_title) for t in Themes.query.order_by('theme_title')]
    if form.validate_on_submit():
        existing_theme = Articles.query.filter_by(
            article_title=form.article_title.data).first()
        if existing_theme is None:
            article = Articles(
